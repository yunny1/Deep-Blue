...
echo "═══════════════════════════════════════════════════════"
echo "🚢 海运航线距离表 — 替换 Haversine"
echo "═══════════════════════════════════════════════════════"

# ━━━ Step 1: 创建海运距离查找表 ━━━
echo ">>> Step 1/2: 创建 sea-route-distances.ts"
cat > "$P/src/lib/sea-route-distances.ts" << 'DISTEOF'
/**
 * 海运航线距离查找表（单位：公里）
 * 
...
DISTEOF
echo "  ✅ sea-route-distances.ts (210 对海运距离)"

# ━━━ Step 2: 更新 BRICSInvestmentPanel — 使用海运距离 ━━━
echo ""
echo ">>> Step 2/2: 更新 BRICSInvestmentPanel.tsx"
python3 << 'PYEOF'
...
# 2a. 添加 import
old_import = "import { estimateSubseaCapex, formatUsd, SENSITIVITY_ITEMS } from '@/lib/subsea-cost-model';"
new_import = """import { estimateSubseaCapex, formatUsd, SENSITIVITY_ITEMS } from '@/lib/subsea-cost-model';
import { getSeaRouteDistance, isLandlocked, LANDLOCKED_PORTS } from '@/lib/sea-route-distances';"""
c = c.replace(old_import, new_import)

# 2b. 删除 haversine 函数定义
old_haversine = """function haversineKm(lat1:number,lon1:number,lat2:number,lon2:number):number{
  const R=6371,toR=(d:number)=>d*Math.PI/180;
  const dLat=toR(lat2-lat1),dLon=toR(lon2-lon1);
  const a=Math.sin(dLat/2)**2+Math.cos(toR(lat1))*Math.cos(toR(lat2))*Math.sin(dLon/2)**2;
  return R*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));
}"""
c = c.replace(old_haversine, "")

# 2c. 替换距离计算
old_dist = "const dist=useMemo(()=>Math.round(haversineKm(fm?.center[1]??0,fm?.center[0]??0,tm?.center[1]??0,tm?.center[0]??0)*1.3),[from,to]);"
new_dist = "const {km:dist,source:distSrc}=useMemo(()=>getSeaRouteDistance(from,to),[from,to]);"
c = c.replace(old_dist, new_dist)

# 2d. 更新距离显示区域 — 去掉 ×1.3 说明，改为海运距离说明
old_label = "{isZh?'估算路由距离（×1.3系数）':'Est. route (×1.3 factor)'}"
new_label = "{isZh?'海运航线距离':'Sea Route Distance'}"
c = c.replace(old_label, new_label)

# 也检查是否有另一个版本的标签
old_label2 = "{isZh?'路由距离':'Route'}"
new_label2 = "{isZh?'海运航线距离':'Sea Route'}"
c = c.replace(old_label2, new_label2)

# 2e. 在距离显示旁加内陆国提示
# 找到距离显示数字的 div 并在后面加一个内陆国提示
old_dist_display = """{dist.toLocaleString()} km</div>
        </div>"""
new_dist_display = """{dist.toLocaleString()} km</div>
...
c = c.replace(old_dist_display, new_dist_display)

# 2f. 更新 SVG 底部距离标注
old_svg_label = "{isZh?'深海电缆':'Deep Sea Cable'} — {dist.toLocaleString()} km"
new_svg_label = "{cn(from,isZh)} → {cn(to,isZh)} — {dist.toLocaleString()} km"
if old_svg_label in c:
    c = c.replace(old_svg_label, new_svg_label)

with open(path, 'w') as f:
    f.write(c)
print("  ✅ BRICSInvestmentPanel.tsx 已更新为海运距离")
print(f"    - 删除 Haversine 函数")
print(f"    - 距离来源: getSeaRouteDistance()")
print(f"    - 内陆国显示接驳港口提示")
PYEOF
...
echo "  git add -A && git commit -m 'feat: replace Haversine with sea route distance table (210 pairs)' && git push origin main"

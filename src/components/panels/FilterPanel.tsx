...
export default function FilterPanel() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cables/filter-options?${buildQueryParams()}`)
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      fetch(`/api/cables/filter-options?${buildQueryParams()}`)
        .then(r => r.json()).then(d => setOptions(d)).catch(() => {});
    }, 400);
  ...
  if (colorMode === 'status' && options?.statuses) {
    ...
    options.vendors.forEach((v: any) => {
      primaryItems.push({ key: v.name, label: v.name === '__other__' ? (zh ? '其他' : 'Others') : v.name, color: VENDOR_COLORS[v.name] || '#6B7280', count: v.count });
    });
  } else if (colorMode === 'operator' && options?.operators) {
    options.operators.forEach((o: any) => {
      primaryItems.push({ key: o.name, label: o.name === '__other__' ? (zh ? '其他' : 'Others') : o.name, color: OPERATOR_COLORS[o.name] || '#6B7280', count: o.count });
    });
  }
  ...
  const primaryDimensionLabel = colorMode === 'status'
    ? (zh ? '运行状态' : 'Status')
    : colorMode === 'vendor'
    ? (zh ? '建造商' : 'Builder')
    : (zh ? '运营商' : 'Operator');
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .filter-scroll::-webkit-scrollbar-thumb { background: rgba(42,157,143,0.3); border-radius: 2px; }
        .filter-scroll::-webkit-scrollbar-thumb:hover { background: rgba(42,157,143,0.6); }
      `}</style>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {activeFilterCount > 0 && (
              <span style={{ fontSize: 9, fontWeight: 700, color: '#E9C46A', backgroundColor: 'rgba(233,196,106,0.15)', border: '1px solid rgba(233,196,106,0.3)', borderRadius: 8, padding: '1px 6px' }}>
                {activeFilterCount} {zh ? '项筛选' : 'active'}
              </span>
            )}
          </div>
            ...
            {colorMode !== 'year' && (
              <>
                ...
                {loadingOptions && !options ? (
                  <div style={{ padding: '12px 0', textAlign: 'center', color: '#4B5563', fontSize: 11 }}>
                    {zh ? '加载中...' : 'Loading...'}
                  </div>
                ) : (
                  <div className="filter-scroll" style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 14, maxHeight: 200, overflowY: 'auto', paddingRight: 2 }}>
                    {primaryItems.map(item => {
                      ...
                      return (
                        <div key={item.key} onClick={() => togglePrimary(item.key)} style={{
                          ...
                          <span style={{ fontSize: 11, color: active ? '#D1D5DB' : '#4B5563', flex: 1, fontWeight: active ? 500 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{item.label}</span>
                  ...
                  <div>
                    <div style={{ fontSize: 10, color: '#6B7280', marginBottom: 6, fontWeight: 600 }}>
                      ...
                      {filterVendors.length > 0 && (
                        <span onClick={() => setFilterVendors([])} style={{ marginLeft: 8, color: '#E9C46A', cursor: 'pointer', fontWeight: 400 }}>
                          {zh ? '清除' : 'Clear'}
                        </span>
                      )}
                    </div>
                      ...
                      {options.vendors.slice(0, 8).map((v: any) => {
                        ...
                        return (
                          <div key={v.name} onClick={() => {
                            ...
                            <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: color }} />
                            {v.name === '__other__' ? (zh ? '其他' : 'Others') : v.name}
                          </div>
                        );
                      })}
                    </div>

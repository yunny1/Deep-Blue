...
const VENDOR_LEGEND = [
  { label: 'ASN (Nokia)', color: '#3B82F6' }, { label: 'SubCom', color: '#EF4444' },
  { label: 'NEC', color: '#F59E0B' }, { label: 'HMN Tech', color: '#10B981' },
  { label: 'Prysmian', color: '#8B5CF6' }, { label: 'Ericsson', color: '#EC4899' },
  { label: 'Xtera', color: '#06B6D4' }, { label: 'Nexans', color: '#F97316' },
  { labelKey: 'color.others', color: '#6B7280' },
];
const OPERATOR_LEGEND = [
  { label: 'Google', color: '#34A853' }, { label: 'Orange', color: '#FF7900' },
  { label: 'BT', color: '#6400AA' }, { label: 'Sparkle', color: '#E91E63' },
  { label: 'Vodafone', color: '#E60000' }, { label: 'Meta', color: '#1877F2' },
  { label: 'Telekom Malaysia', color: '#0054A6' }, { label: 'Tata / Singtel', color: '#00BCD4' },
  { labelKey: 'color.others', color: '#6B7280' },
];
const YEAR_LEGEND = [
  { labelKey: 'color.before2000', color: '#6B7280' }, { label: '2000-2009', color: '#F59E0B' },
  { label: '2010-2019', color: '#3B82F6' }, { labelKey: 'color.new2020', color: '#10B981' },
...
export default function ColorControlPanel() {
  ...
  const legend =
    colorMode === 'status'   ? STATUS_LEGEND   :
    colorMode === 'vendor'   ? VENDOR_LEGEND   :
    colorMode === 'operator' ? OPERATOR_LEGEND :
    YEAR_LEGEND;

  return (
    <div style={{
      position: 'absolute',
      // 往下移：导航栏56 + NewsTicker32 + 8间距 = 96px，不再和NewsTicker重叠
      top: 96,
      left: 16,
      ...
      <div style={{ fontSize: 11, fontWeight: 600, color: '#2A9D8F', marginBottom: 10, textTransform: 'uppercase' as const, letterSpacing: 1 }}>
        {t('color.title')}
      </div>
      ...
      <div style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', marginBottom: 8, textTransform: 'uppercase' as const, letterSpacing: 1 }}>
        {t('color.legend')}
      </div>

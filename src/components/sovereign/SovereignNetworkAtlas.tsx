// 这是对 SovereignNetworkAtlas.tsx 的局部补丁说明
// 需要在现有 v5 的基础上做以下几处修改：

// ① 新增 state（在已有 state 声明区域添加）：
//   const mapRef = useRef<HTMLDivElement>(null);       // 用于滚动到地图
//   const [highlightedCable, setHighlightedCable] = useState<string|null>(null);

// ② 删除地图上方的 xlsx 上传 label（那段 <label> 元素整块删除）

// ③ 修改 SovereignNetworkMap 的调用，增加两个 prop：
//   highlightedCableName={highlightedCable}
//   // 给地图容器加一个 ref wrapper
//   先把 <SovereignNetworkMap> 外面包一个 <div ref={mapRef}>

// ④ AllCablesTable 的 onCableClick 回调里，触发滚动+高亮：
//   const handleTableCableClick = useCallback((name:string, score:number, routeCount:number) => {
//     openCableModal(name, score, routeCount);   // 打开详情 modal（已有）
//     setHighlightedCable(name);                  // 高亮地图上这条缆（新增）
//     mapRef.current?.scrollIntoView({ behavior:'smooth', block:'center' }); // 滚动到地图
//   }, [openCableModal]);
//   然后把 AllCablesTable 的 onCableClick 换成 handleTableCableClick

// ⑤ 在 handleSelect（路径选中）里，清除 highlightedCable：
//   const handleSelect = useCallback((id:string|null) => {
//     setSelectedId(id);
//     if (!id) setRoutePopup(null);
//     setHighlightedCable(null);   // ← 新增这行
//   }, []);

// ⑥ 动态加载路径数据（在 useEffect 里，组件挂载后从 Redis 读最新数据）：
//   useEffect(() => {
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 下面是完整的修改后版本（基于 v5，集成了所有上述变更）
'use client';
// src/components/sovereign/SovereignNetworkAtlas.tsx  v6
...
function Badge({safety}:{safety:SafetyLevel}){
  ...
  return<span style={{background:bg,color:text,border:`1px solid ${border}`,fontSize:10,padding:'1px 7px',borderRadius:20,fontWeight:600,whiteSpace:'nowrap',flexShrink:0}}>{label}</span>;
}
function Dot({score,size=6}:{score:number;size?:number}){
  const c=riskColor(score);
  return<span style={{width:size,height:size,borderRadius:'50%',background:c,display:'inline-block',flexShrink:0,boxShadow:`0 0 ${size}px ${c}60`}}/>;
}
function AnimNum({n,color=GOLD_LIGHT}:{n:number;color?:string}){
  const[v,setV]=useState(0);
  useEffect(()=>{const t0=Date.now();const tick=()=>{const p=Math.min((Date.now()-t0)/1200,1);setV(Math.round(n*(1-Math.pow(1-p,3))));if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);},[n]);
...
export default function SovereignNetworkAtlas(){
  ...
  const handleUpload=async(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    for(const r of parsed)r.cables.split(' | ').forEach(c=>rawNames.add(c.trim()));
    setNormalizing(true);setNormalizeMsg(`正在调用 AI 归一化 ${rawNames.size} 个海缆名称…`);
    let nameMapping:Record<string,string>={};
    try{
      const res=await fetch('/api/sovereign/normalize-cables',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({names:[...rawNames]})});
      const nd=await res.json();nameMapping=nd.mapping??{};
      const normalizedCount=Object.entries(nameMapping).filter(([k,v])=>k!==v).length;
      setNormalizeMsg(`✓ AI 去重完成：${rawNames.size} 个名称归一化为 ${new Set(Object.values(nameMapping)).size} 条标准海缆`+(normalizedCount>0?`（修正了 ${normalizedCount} 个变体）`:'（名称均已标准）'));
    }catch(err){setNormalizeMsg('⚠ AI 去重失败，将使用原始名称');console.warn('[normalize]',err);}
    const normalizedRoutes=parsed.map(r=>({...r,cables:r.cables.split(' | ').map(c=>nameMapping[c.trim()]??c.trim()).join(' | ')}));
    // 保存到 Redis（通过管理接口）
    try{
      const saveRes=await fetch('/api/admin/sovereign-routes-upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({routes:normalizedRoutes})});
      if(saveRes.ok){const d=await saveRes.json();setNormalizeMsg(prev=>prev+` · ${d.message}`);}
    }catch(e){console.warn('[save routes]',e);}
  ...
  const openCableModal=useCallback((cableName:string,score:number,routeCount:number)=>{
    ...
    const dbRecord=cableByName.current.get(key)??abbrs.reduce<CableDbRecord|undefined>((f,a)=>f??cableByName.current.get(a),undefined);
    setCableModal({name:cableName,slug:dbRecord?.slug??'',score,routeCount,stations:dbRecord?.stations??[]});
  },[]);

  // 表格里点击缆：打开详情 + 高亮地图 + 滚动到地图
  const handleTableCableClick=useCallback((name:string,score:number,routeCount:number)=>{
  ...
  const handleSelect=useCallback((id:string|null)=>{
    setSelectedId(id);
    if(!id)setRoutePopup(null);
    setHighlightedCable(null); // 选中路径时清除单条缆高亮
  },[]);
      ...
      <div style={{maxWidth:1400,margin:'0 auto',padding:'32px 32px 48px'}}>
        ...
        <div className="sna-up" style={{marginBottom:28}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            ...
            <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'6px 14px',background:`${GOLD}08`,border:`1px solid ${GOLD}1e`,borderRadius:20}}>
          ...
          <h1 style={{fontSize:36,fontWeight:800,color:GOLD_LIGHT,margin:'0 0 8px',lineHeight:1.1}}>自主权网络图谱</h1>
        ...
        <div className="sna-up" style={{display:'flex',gap:16,animationDelay:'.12s',alignItems:'flex-start'}}>
          ...
          <div style={{width:264,flexShrink:0,display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div className="sna-card" style={{padding:16}}>
              ...
              {[
                ...
                {label:'起点国家',val:filterFrom,set:(v:string)=>{setFilterFrom(v);setFilterTo('');},opts:fromOpts.map(c=>({v:c,l:c}))},
          ...
          <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',gap:12}}>
            ...
            {highlightedCable&&!selectedRoute&&(
              <div style={{padding:'8px 14px',borderRadius:8,background:'rgba(255,215,0,.08)',border:'1px solid rgba(255,215,0,.3)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <span style={{fontSize:12,color:'#FFD700'}}>
                  正在高亮：{highlightedCable}
                  <span style={{color:'rgba(255,255,255,.35)',marginLeft:8,fontSize:11}}>点击地图空白处取消</span>
                </span>
            ...
            {selectedRoute
              ...
              :<AllCablesTable cables={allUniqueCables} total={routes.length} filtered={filtered.length} onCableClick={handleTableCableClick}/>}
          </div>
        </div>
        ...
        <footer style={{marginTop:40,paddingTop:18,borderTop:`1px solid ${GOLD}0e`}}>
          <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'rgba(255,255,255,.18)',marginBottom:8}}>
            ...
            <span>保留海缆共 {CANONICAL_CABLE_NAMES.length} 条 · 路径数据更新请前往管理后台</span>
          </div>
        </footer>
      </div>
...
// ── SelectedRouteDetail（与 v5 完全相同，保留分支路线展示）─────────────────
  ...
  return(
    <div style={{background:CARD_BG,border:`1px solid ${GOLD_DIM}`,borderRadius:14,backdropFilter:'blur(12px)',padding:'20px 24px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:10,marginBottom:20}}>
        {[{l:'最大段风险',v:route.maxRisk,c:riskColor(route.maxRisk)},{l:'平均风险',v:route.avgRisk,c:riskColor(route.avgRisk)},{l:'保留段数',v:route.segments,c:GOLD_LIGHT}].map(({l,v,c})=>(
      ...
      <div style={{fontSize:11,fontWeight:600,letterSpacing:'.06em',textTransform:'uppercase',color:`${GOLD}80`,marginBottom:12}}>各段海缆明细（点击查看详情）</div>
      {segmentData?(
              ...
              <div key={seg.seg} style={{display:'flex',alignItems:'stretch',gap:0}}>
                ...
                <div style={{flex:1,paddingBottom:isLast?0:12,paddingLeft:8}}>
                  ...
                  {hasMulti?(
                    <div style={{position:'relative',paddingLeft:16}}>
                      ...
                      {seg.cables.map((cable,cIdx)=>(
                        <div key={cIdx} style={{position:'relative',marginBottom:cIdx<seg.cables.length-1?6:0}}>
                          ...
                          <button onClick={()=>onCableClick(cable.name,cable.score,allCables.find(c=>c.name.toLowerCase()===cable.name.toLowerCase())?.routeCount??1)}
                    ...
                    seg.cables.map((cable,cIdx)=>(
                      <button key={cIdx} onClick={()=>onCableClick(cable.name,cable.score,allCables.find(c=>c.name.toLowerCase()===cable.name.toLowerCase())?.routeCount??1)}
                        style={{display:'flex',alignItems:'center',justifyContent:'space-between',width:'100%',padding:'7px 12px',cursor:'pointer',textAlign:'left',
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {route.cables.split(' | ').map((cable,i)=>{const scores=route.riskScores.split(' | ').map(Number);const score=scores[i]??route.maxRisk;return(
            <button key={i} onClick={()=>onCableClick(cable.trim(),score,allCables.find(c=>c.name.toLowerCase()===cable.trim().toLowerCase())?.routeCount??1)}
    ...
    <div style={{background:CARD_BG,border:`1px solid ${GOLD_DIM}`,borderRadius:14,backdropFilter:'blur(12px)',padding:'20px 24px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div>
          ...
          <h2 style={{fontSize:18,fontWeight:700,color:GOLD_LIGHT,margin:'0 0 2px',fontFamily:"'Playfair Display',serif"}}>当前范围内所有保留海缆</h2>
          <p style={{fontSize:11,color:'rgba(255,255,255,.3)',margin:0}}>点击任意行：查看详情 + 地图高亮定位</p>
        </div>
          ...
          <thead>
            <tr style={{borderBottom:'1px solid rgba(255,255,255,.08)'}}>
              {['海缆名称','风险评分','出现路径数','评级'].map(h=>(
                <th key={h} style={{padding:'8px 10px',textAlign:h==='海缆名称'?'left':'center',fontSize:10,fontWeight:600,letterSpacing:'.06em',textTransform:'uppercase',color:`${GOLD}70`}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={i} onClick={()=>onCableClick(cable.name,cable.score,cable.routeCount)}
                  ...
                  <td style={{padding:'10px 10px',color:'rgba(255,255,255,.85)'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      ...
                      {cable.name}
                      <span style={{fontSize:10,color:'rgba(255,255,255,.25)',marginLeft:2}}>↗ 地图定位</span>
                    </div>
                  </td>
      ...
      <div style={{position:'fixed',left:'50%',top:'50%',transform:'translate(-50%,-50%)',
        ...
        <div style={{padding:'18px 20px',borderBottom:`1px solid ${GOLD}12`,display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div style={{flex:1,overflow:'hidden',paddingRight:12}}>
            <div style={{fontSize:11,color:`${GOLD}80`,fontWeight:600,letterSpacing:'.08em',textTransform:'uppercase',marginBottom:5}}>海缆详情</div>
        ...
        <div style={{padding:'14px 20px',display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
          {[{l:'风险评分',v:data.score,c:color},{l:'出现路径数',v:data.routeCount,c:GOLD_LIGHT},{l:'登陆站数量',v:data.stations.length,c:GOLD_LIGHT}].map(({l,v,c})=>(
        ...
        {data.stations.length===0&&<div style={{padding:'0 20px 18px',fontSize:12,color:'rgba(255,255,255,.3)'}}>登陆站数据加载中，或该缆暂未收录在数据库</div>}
        <div style={{padding:'12px 20px',borderTop:`1px solid ${GOLD}10`,textAlign:'right'}}>
          <button onClick={onClose} style={{padding:'7px 20px',borderRadius:7,background:`${GOLD}12`,border:`1px solid ${GOLD}30`,color:GOLD,cursor:'pointer',fontSize:12,fontWeight:500}}>关闭</button>
        </div>
      </div>
    </>
  );
  ...
  return(
    <>
      ...
      <div style={{position:'fixed',left:Math.min(info.x+16,window.innerWidth-340),top:Math.max(info.y-60,12),
        width:320,background:'rgba(8,18,36,.97)',backdropFilter:'blur(20px)',border:`1px solid ${GOLD}25`,
        borderRadius:12,zIndex:201,boxShadow:'0 8px 40px rgba(0,0,0,.7)',overflow:'hidden'}}>
        ...
        <div style={{padding:'10px 16px 14px',maxHeight:280,overflowY:'auto'}}>
          <div style={{fontSize:10,fontWeight:600,letterSpacing:'.06em',textTransform:'uppercase',color:`${GOLD}70`,marginBottom:10}}>涉及海缆（{info.cables.length} 条）</div>
          {info.cables.map((c,i)=>(

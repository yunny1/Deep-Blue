...
import { isLand, OCEAN_MASK_COLS, OCEAN_MASK_ROWS, OCEAN_MASK_HEX, OCEAN_MASK_RESOLUTION } from './ocean-mask';
...
function latLonToGrid(lat: number, lon: number): [number, number] {
  const row = Math.floor((90 - lat) / OCEAN_MASK_RESOLUTION);
  let col = Math.floor((lon + 180) / OCEAN_MASK_RESOLUTION);
...
function gridToLatLon(row: number, col: number): [number, number] {
  return [
    90 - (row + 0.5) * OCEAN_MASK_RESOLUTION,
    -180 + (col + 0.5) * OCEAN_MASK_RESOLUTION,
  ];
}
...
function snapToOcean(lat: number, lon: number): [number, number] {
  ...
  // 螺旋搜索最近的海洋格子（最远搜索 80 格 = 20° at 0.25° resolution）
 ...
 * 只在海洋格子上移动
 * 使用 binary heap 优化，支持 0.25° 网格上的长距离路由
 */
function astarPath(
  ...
  const heap: { key: number; row: number; col: number; f: number }[] = [];
  const gScore = new Map<number, number>();
  const cameFrom = new Map<number, number>();
  const closedSet = new Set<number>();

  function heapPush(item: typeof heap[0]) {
    heap.push(item);
    let i = heap.length - 1;
    while (i > 0) {
      const parent = (i - 1) >> 1;
  ...
  function heapPop(): typeof heap[0] {
    const top = heap[0];
    const last = heap.pop()!;
    if (heap.length > 0) {
      heap[0] = last;
  ...
  heapPush({ key: startKey, row: startRow, col: startCol, f: heuristic(startRow, startCol, endRow, endCol) });
  ...
  while (heap.length > 0 && iterations < maxIterations) {
    ...
    for (const [dr, dc] of DIRS) {
      ...
      if (tentativeG < (gScore.get(neighborKey) ?? Infinity)) {
        ...
        // Always push to heap (duplicates are handled by closedSet check)
        heapPush({ key: neighborKey, row: nr, col: nc, f });
      }
    }
  }
...
// ============================================================
// 4. Path decimation (no Douglas-Peucker — it cuts corners over land)
// ============================================================

/**
 * Simple decimation: keep every Nth point to reduce density
 * Always keeps first and last point
 * Unlike Douglas-Peucker, this never introduces shortcuts that cross land
 */
function decimatePath(path: [number, number][], keepEvery: number = 3): [number, number][] {
  if (path.length <= 2) return path;
  const result: [number, number][] = [path[0]];
  for (let i = keepEvery; i < path.length - 1; i += keepEvery) {
    result.push(path[i]);
  }
...
export function generateApproximateRoute(
  ...
  for (let i = 0; i < ordered.length - 1; i++) {
    ...
    // 简单抽稀（保留每 3 个点中的 1 个，不做 Douglas-Peucker 避免切角）
    const decimated = decimatePath(gridPath, 3);

    // 转换为经纬度坐标
    const waypoints = decimated.map(([r, c]) => gridToLatLon(r, c));
